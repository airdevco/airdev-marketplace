        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {pricingPlans.map((plan) => (
            ...
            >
                ...
                {plan.features.map((feature, index) => (
                  <li key={index} className="flex items-start gap-3">
                    <div className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center mt-0.5 ${
                      plan.highlighted ? "bg-[#1265EF]" : "bg-[#1265EF]/10"
                    }`}>
                      <Check className={`w-3 h-3 ${plan.highlighted ? "text-white" : "text-[#1265EF]"}`} />
                    </div>
              ...
              {/* CTA Button */}
              <div className={`${plan.highlighted ? "mt-auto pt-8" : "mt-8"}`}>
                <a
                  href="#talk-to-us"
                  className={`block w-full text-center py-3.5 px-6 rounded-lg font-medium transition-all duration-200 ${
                    plan.highlighted
                      ? "bg-[#1265EF] text-white hover:bg-[#0d4fc7]"
